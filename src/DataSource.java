...
public class DataSource {
	...
	public List<Patient> getAlarmingAgePatients(String testName) {
		...
		try {
			...
			try {
				...
				// Ignore view does not exist;
			} finally {
				stmt.executeUpdate("CREATE VIEW medical_risk(medical_type,alarming_age,abnormal_rate) AS "
			...
			rs = stmt.executeQuery("SELECT DISTINCT name, address, phone "
	...
	public boolean checkTestName(String testName) {
		try {
			String query = "SELECT type_id FROM test_type t WHERE UPPER(t.test_name) = UPPER('" + testName + "')";
